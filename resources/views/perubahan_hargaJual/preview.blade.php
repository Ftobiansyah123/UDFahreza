        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">{{ __('Cetak Perubahan Harga') }}
                    <div class="float-end">
                        <form method="GET" action="{{ route('perubahan_hargaJual.preview') }}">
                            <label for="start_date">Mulai Tanggal:</label>
                            <input type="date" id="start_date" name="start_date">
                            <label for="end_date">Akhir Tanggal:</label>
                            <input type="date" id="end_date" name="end_date">
                            <button class="btn btn-success" type="submit">Cek Data</button>
                             <a href="{{ route('perubahan_hargaJual.cetakPDF') }}?start_date={{ request('start_date') }}&end_date={{ request('end_date') }}" type="submit" class="btn btn-primary">Cetak PDF</a>
                        </form>
                    ...
                    <div class="container">
                       ...
                       <div>
                           <table class="table table-bordered">
                               ...
                               <!-- Tambahkan kode untuk memfilter data berdasarkan periode yang dipilih -->
                               @foreach ($perubahan_hargaJual as $ph)
                                   ...
                                   <tr>
                                       ...
                                       <td>{{ $ph->tanggal }}</td>
                                       <td class="price-detail">{{ $ph->harga_modal}}</td>
                                       <td class="price-detail">{{ $ph->harga_jual }}</td>
                                   </tr>
                               </tbody>
                               @endforeach
                               <tfoot>
